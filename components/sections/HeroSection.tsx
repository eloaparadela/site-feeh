...
export default function HeroSection() {
  ...
  return (
    <section id="hero" aria-label="Apresentação Prosat">
        ...
        <div className="hidden md:block absolute inset-0 z-10 pointer-events-none">
          <div className="absolute bottom-16 left-8 lg:left-16 xl:left-24 w-max max-w-[calc(100vw-4rem)] pointer-events-auto">
            ...
            <div className="max-w-[500px] bg-white/60 backdrop-blur-md border-l-4 border-[#B4995A] px-6 py-5 lg:px-7 lg:py-6 shadow-2xl shadow-black/30">
              <h1 className="text-lg lg:text-xl xl:text-2xl font-bold text-[#0F0A0F] leading-snug mb-2">
                {slide.title}
              </h1>
            ...
            {/* CTAs — presos ao cartão, sobre a foto, sempre lado a lado */}
            <div className="mt-4 flex flex-nowrap items-center gap-2.5">
              ...
              <a
                ...
                className="inline-flex shrink-0 items-center justify-center gap-2 whitespace-nowrap bg-[#0F0A0F]/75 backdrop-blur-sm border border-[#B4995A]/60 hover:border-[#B4995A] text-[#B4995A] hover:bg-[#B4995A]/15 font-semibold text-[15px] px-5 py-4 rounded shadow-lg shadow-black/40 transition-colors focus-ring"
              >
      ...
      {/* ═══════════════════════════════════════
          BOTÕES DE AÇÃO — mobile (no desktop ficam sobre o hero)
      ══════════════════════════════════════ */}
      <div className="md:hidden bg-[#0F0A0F] px-4 pt-1 pb-4 flex flex-col gap-2.5">
        ...
        <a
          href={slide.tertiaryButtonLink}
          className="inline-flex items-center justify-center gap-2 bg-[#B4995A]/10 border border-[#B4995A]/60 hover:border-[#B4995A] text-[#B4995A] hover:bg-[#B4995A]/20 font-semibold text-[15px] px-6 py-3.5 rounded transition-colors focus-ring w-full"
        >
          {slide.tertiaryButtonLabel}
          <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24" aria-hidden="true">
            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 5l7 7-7 7" />
      ...
      {/* ── Dots + setas mobile ── */}
      <div className="md:hidden bg-[#0F0A0F] flex items-center justify-center gap-4 pb-5 pt-1">
        <button
          onClick={() => { prev(); setPaused(true) }}
          aria-label="Slide anterior"
